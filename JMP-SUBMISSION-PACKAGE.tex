...
\item Non-zero mass gap confirmed: $\Delta m = 64 \hbar\omega$ for Yang--Mills with weak coupling $g=0.1$
...
\textbf{Included:}
\begin{itemize}
\item \texttt{mass\_gap\_computation.py}: 469-line Python module with Yang-Mills equations integrated throughout
...
\item Ready for \verb|\bibliography| command
\end{itemize}
...
\textit{Mass gap vs. coupling $g$ over range} $[0.01, 0.5]$: Power law fit gives $\Delta m \propto g^{0.00}$

Interpretation: Mass gap is topologically protected and independent of coupling to leading order, reflecting the Yang-Baxter structure underlying the Yang--Mills gauge fixing.
...
\begin{enumerate}
\item \textbf{Main manuscript:} \texttt{yang-mills-complete-AIP.tex} and compiled PDF
   \begin{itemize}
   \item REVTeX 4.1 format for AIP Publishing journals
   \item Optimized for Journal of Mathematical Physics (JMP)
   \end{itemize}
...
\begin{itemize}
\item \textbf{Manuscript inquiries:} [drewremmenga@gmail.com]
\item \textbf{Code repository:} \url{https://github.com/dremmeng/mass-gap-spectra}
